...
router.post('/block-user', authMiddleware, async (req, res) => {
    try {
        const { chatId, blockedBy } = req.body;
        ...
        const updatedChat = await Chat.findByIdAndUpdate(chatId, { isBlocked: true, blockedBy: blockedBy }, { new: true }).populate('members').populate('lastMessage');
        ...
        console.log('Error in block-user controller.');
        res.status(500).send({
            message: error.message,
            success: false
        });
...
router.post('/unblock-user', authMiddleware, async (req, res) => {
    try {
        ...
        const updatedChat = await Chat.findByIdAndUpdate(chatId, { isBlocked: false, blockedBy: null }, { new: true }).populate('members').populate('lastMessage');
        ...
        console.log('Error in unblock-user controller.');
        res.status(400).send({
            message: error.message,
            success: false
        });
